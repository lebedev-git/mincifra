...
// с авто-статусами G3 (из отчёта проверок).
function buildTracker(id) {
  ...
  const gates = GATES.map((g) => {
    const items = g.items.map((it) => {
      const done = g.auto ? !!auto[it.id] : !!manual[it.id];
      itemsTotal++; if (done) doneTotal++;
      return { ...it, done };
    });
    const done = items.filter((i) => i.done).length;
    return {
      id: g.id, title: g.title, auto: !!g.auto,
...
function updateManual(id, patch) {
  const manual = store.getTracker(id) || {};
  const autoIds = new Set(GATES.filter((g) => g.auto).flatMap((g) => g.items.map((i) => i.id)));
  for (const [k, v] of Object.entries(patch || {})) {
    if (autoIds.has(k)) continue; // авто-пункты не редактируются вручную
    manual[k] = !!v;
  }
...
module.exports = { GATES, buildTracker, updateManual, autoStateFromReport };
